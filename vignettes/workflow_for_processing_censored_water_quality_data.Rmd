---
title: "Vignette, Workflow for Processing Censored Water Quality Data Sets"
...
output: 
  rmarkdown::html_vignette:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{Vignette, Workflow for Processing Censored Water Quality Data Sets}
...
## Purpose
The purpose of this vignette is to demonstrate a workflow for managing data sets that have censored data. In the context of water quality data, we often think of censored values reported as not-detected, less-than the quantitation limit, between detection limit and limit of quantitation to name a few. For example, in water quality, the value “<0.5” is typically thought to have a lower and upper bound of 0 and 0.5, respectively. The package `baytrends` relies on the `survival` package which treats these values as “interval” censored data and are referred to as *Surv objects*. 
...
Importing CSV files that have censored data into R dataframes for analysis in baytrends is a critical data preparation step and involves **two lines of code**: 1) importing the CSV data into R and 2) converting fields with censored data to Surv objects (see below code).
