...
2017-12-20, more functions after adding in all of swmrQW functions.  Only testing original examples.
...
```
## Rename all
For final version of package rename all "x_*" scripts to "smwrQW_x_*".  Want to keep the latest additions separate from the previously added smwrQW scripts.  The previous ones are known to be needed the "x" ones not as sure but the package works with them.  There are 85 "x" scripts out of 123 in the "R" folder.

```{r, eval=FALSE}
wd <- "C://Users//Erik.Leppo//OneDrive - Tetra Tech, Inc//MyDocs_OneDrive//GitHub//baytrends//R"
#
fn.orig <- list.files(path=wd, pattern="x_")
fn.new <- paste0("smwrQW_",fn.orig)
file.rename(file.path(wd,fn.orig), file.path(wd, fn.new))
```
